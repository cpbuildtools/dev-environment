...
async function showClonedContainerOptions(repo: string) {
  const devCont = (await buildContainerChoices()).find(
    (c) => c.value.shortDir === repo
  );
  if (devCont) {
...
async function showContainerWorkspaceMenu(container: ContainerMenuItem) {
  const choices = (await findDevContainerWorkspaces(container.rootDir)).map(
    (ws) => {
      const file = parse(ws);
      return {
        type: "choice",
...
async function findDevContainerFiles() {
  ...
  containerRoot = resolve(containerRoot);
  return await glob("**/.devcontainer/devcontainer.json", {
    cwd: containerRoot,
    dot: true,
  });
}
...
async function findDevContainerWorkspaces(containerFolder: string) {
  return await glob("**/*.code-workspace", {
    cwd: join(containerFolder, "workspaces"),
  });
}
