...
export const configPropertyChangeHandlers: { [property: string]: PropertyChangeHandler } = {
...
export const builder = (yargs: Argv) => {
    return yargs
        .command('get <property>', 'Gets a config property',
            builder => builder
                .positional('property', { type: 'string' }),
            args =>
                printConfig(args.property))
        .command('set <property> <value>', 'Sets a config property',
            builder => builder
                .positional('property', { type: 'string' })
                .positional('value', { type: 'string' }),
            args =>
                printConfig(args.property, args.value))
        .command('show', 'Shows the config object',
            builder => builder,
            _ => printConfig())
        .option('set', {
            type: 'string'
...
export async function setArgsToConfig(setValues?: string[]) {
    const config = {};
    const setExps = Array.isArray(setValues) ? setValues.map(i => i.split('=')) : [];
    setExps.forEach(i => {
...
export async function promptConfig(setValues?: string[], skipPrompt: boolean = false) {
...
async function handleConfigPropertyChanges(action: 'add' | 'remove' | 'change', keys: string[], config: { [k: string]: string }) {
